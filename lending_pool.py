...
class LendingPool:
    ...
    def issue_loan(self, collateral, interest_rate, maturity, loan_to_value=0.75, liquidation_ratio=0.8, safety_margin=0.05):
    ...
    def simulate(self, num_days, last_price, volatility, expected_return, num_simulations):
        for x in range(num_simulations):
            ...
            for loan in self.loans:
                loan.value_over_time = []  # Reset the value_over_time list at the start of each simulation
                option_price = black_scholes(price_series, loan.strike_price, loan.maturity, loan.interest_rate, volatility, option_type='put')
                num_contracts = loan.collateral / 1  # Deribit options contracts are for 1 ETH
                total_option_value = option_price * price_series * num_contracts  # Multiply by the price of ETH to get the total value in USD
...
pool.simulate(num_days=365, last_price=1957, volatility=0.4, expected_return=0.05, num_simulations=1)
